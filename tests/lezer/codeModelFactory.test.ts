...
import { LezerCodeModelFactory } from '../../src/bsl/lezer/factory/codeModelFactory'
import { describe, expect, test } from 'vitest'

const codeModelFactory = new LezerCodeModelFactory()
...
describe('Literals', () => {
    ...
    test('Date with custom separators', () => {
        const exp = expression("'0001y01m01d12h13m14s'")
        expect(exp).toBeInstanceOf(ConstSymbol)
        expect(exp).toMatchObject({ value: '00010101121314', type: 'Дата' })
    })
    ...
    test('True', () => {
        ...
        expect(exp).toBeInstanceOf(ConstSymbol)
        expect(exp).toMatchObject({ value: 'true', type: 'Булево' })
    })
...
describe('Try statement', () => {
    ...
    test('base eng', () => {
        const exp = statement('Try a(1);b(1); except c(2);d(2);endTry')
        expect(exp).toBeInstanceOf(TryStatementSymbol)
        expect(exp).toMatchObject({
            body: [{ name: 'a' }, { name: 'b' }],
            handler: [{ name: 'c' }, { name: 'd' }]
...
describe('Preproc', () => {
    test('Var in region', () => {
        const exp = statement('#Область Переменные\nПерем П1;\n#КонецОбласти')
        expect(exp).toBeInstanceOf(VariableDefinitionSymbol)
        expect(exp).toMatchObject({
            vars: [{ name: 'П1' }]
        })
