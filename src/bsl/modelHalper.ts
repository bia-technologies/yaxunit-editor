...
const engLang: boolean = false
function isQueryMode(): boolean {
    return false // TODO
}

function isDCSMode(): boolean {
    return false // TODO
}
...
class bslHelper {
    ...
    lineNumber: number
    column: number
    wordData: editor.IWordAtPosition | null
    word?: string
    lastOperator: string
    hasWhitespace: boolean
    textBeforePosition: string
    lastExpression: string
    lastRawExpression: string
    nameField: string
    queryNameField: string
    token: string

    constructor(editor: editor.IStandaloneCodeEditor, model: editor.ITextModel, position: Position) {
        this.editor = editor
        this.model = model;
     ...
     */
    getLangId(): string {
        return this.editor.getModel()?.getLanguageId() ?? 'bsl'
    }
     ...
     */
    getLastToken(wordData?: editor.IWordAtPosition): string {

        let token = '';
        const column = wordData == null ? this.column : wordData.endColumn + 1;
        const value = this.model.getValueInRange(new Range(1, 1, this.lineNumber, column));
        const lang_id = this.getLangId();
        const tokens = editor.tokenize(value, lang_id);

        if (tokens.length) {
            const last_tokens = tokens[tokens.length - 1];
    ...
    getLastWordWithTokenInRange(token_name: string, startLineNumber: number, startColumn: number, endLineNumber: number, endColumn: number, ignored_words: string[]): string {

        let word = '';

        let value = this.model.getValueInRange(new Range(startLineNumber, startColumn, endLineNumber, endColumn));

        if (value) {

            let model = editor.createModel(value);
            let lang_id = this.getLangId();
            let tokens = editor.tokenize(value, lang_id);
                    ...
                    if (items.length) {
                        ...
                        while (0 <= idx_item && !word) {
                            ...
                            if (0 <= token_type.indexOf(token_name)) {
                                ...
                                let text = model.getWordAtPosition(new Position(idx_line + 1, token.offset + 1));
     ...
     */
    getLastCharInLine(lineNumber: number): string {
        ...
        if (lineNumber) {
            ...
            if (0 < column)
                char = this.model.getValueInRange(new Range(lineNumber, column - 1, lineNumber, column));
    ...
    lastExpressionHasSeparatedParams() {
        ...
        let expArray = this.getExpressionArray();
     ...
     * * @returns {array} array with expressions
     */
    getExpressionArray():string[] {

        let text = this.textBeforePosition;
        const unclosedStringInfo = this.unclosedString(text);
        let unclosed = ''
        if (0 <= unclosedStringInfo.index) {
            text = text.substr(0, unclosedStringInfo.index);
            unclosed = unclosedStringInfo.string;
        }
        else {
            unclosed = '';
        }
    ...
    /**
     * Returns last expression from getExpressionArray - nstep
     * @param {int} nstep - quantity of steps
     */
    getLastNExpression(nstep:number) {
    ...
    getLastSeparatedWord(position) {
        ...
        if (match) {
            ...
            let match_pos = new Position(match.range.startLineNumber, match.range.startColumn);
            ...
            if (match_pos.lineNumber < position.lineNumber || match_pos.lineNumber == position.lineNumber && match_pos.column < position.column) {
                ...
                if (match) {
                    ...
                    let range = new Range(match.range.startLineNumber, match.range.startColumn - 1, match.range.startLineNumber, match.range.startColumn);
                    ...
                    if (range.startLineNumber < position.lineNumber || range.startLineNumber == position.lineNumber && range.startColumn < position.column) {
                        ...
                        if (prevChar == '.') {
                            position = new Position(match.range.startLineNumber, match.range.startColumn);
    ...
    getLastCharacter(range) {
        ...
        if (range == undefined)
            range = new Range(this.lineNumber, this.column - 1, this.lineNumber, this.column);
    ...
    wordHasCharsBefore(chars) {
        ...
        if (this.wordData) {
            let range = new Range(this.lineNumber, data.startColumn - chars.length, this.lineNumber, data.startColumn);
    ...
    wordHasCharsAfter(chars) {
        ...
        if (this.wordData) {
            let range = new Range(this.lineNumber, data.endColumn, this.lineNumber, data.endColumn + chars.length);
     ...
     * @param {CompletionItemKind} kind - languages.CompletionItemKind (class, function, constructor etc.)
     */
    ...
    getRefCompletionFromPosition(suggestions, currentPosition, allowLookBehind) {
        ...
        if (match) {
            ...
            let position = new Position(match.range.startLineNumber, match.range.startColumn);
            ...
            if (position.lineNumber = currentPosition.lineNumber) {
                ...
                if (!suggestions.length && allowLookBehind) {
                    ...
                    if (match) {
                        ...
                        position = new Position(match.range.endLineNumber, match.range.endColumn);
    ...
    getMetadataGeneralMethodCompletionByType(object, methodType, suggestions, kind) {
        ...
        if (object.hasOwnProperty(methodType)) {
            ...
            for (const [mkey, mvalue] of Object.entries(object[methodType])) {
                ...
                suggestions.push({
                    label: mvalue[this.nameField],
                    kind: languages.CompletionItemKind[kind],
                    insertText: mvalue[this.nameField] + postfix,
                    insertTextRules: languages.CompletionItemInsertTextRule.InsertAsSnippet,
                    detail: mvalue.description,
                    documentation: description,
    ...
    getMetadataStackForVar(varName, position) {
        ...
        if (match && match.matches.length == pattern_match_count) {
            ...
            if (match.range.startLineNumber < position.lineNumber) {
                ...
                if (var_match) {
                    this.getStackCompletionForLoopsVar([], source_var, new Position(match.range.startLineNumber, match.range.startColumn));
                }
                ...
                let prev_stack = this.getMetadataStackForVar(source_var, new Position(position.lineNumber, match.range.startColumn));
            ...
            if (var_match) {
                ...
                let range = new Range(position.lineNumber, offset, position.lineNumber, offset + 1);
                let char = this.model.getValueInRange(range);
                let prev_var = this.model.getWordUntilPosition(new Position(position.lineNumber, offset));
                ...
                if (char == '.' && prev_var) {
                    ...
                    let prev_position = new Position(position.lineNumber, offset + 1);
                ...
                for (let i = 3; i < stack.length; i++) {
                    ...
                    if (i == 3) {
                        ...
                        if (stack_item.previous_ref && prev_ref != null) {
                            ...
                            let position = new Position(prev_item.line, prev_item.column + 1);
    ...
    getStackCompletionFromRefs(suggestions, stack) {
        ...
        for (let i = 0; i < stack.length; i++) {
            ...
            if (stack_item.previous_ref && prev_ref != null) {
                ...
                let position = new Position(stack_item.line, stack_item.column + 1);
                if (0 < i) {
                    let prev_item = stack[i - 1];
                    position = new Position(prev_item.line, prev_item.column + 1);
                }
                if (i == 0) {
    ...
    getLoopsVarNamesForCurrentPosition(pattern) {
        ...
        if (loop_start) {
            ...
            let start_pos = new Position(loop_start.range.startLineNumber, loop_start.range.startColumn);
            let loop_end = Finder.findNextMatch(this.model, '(?:конеццикла|enddo)', start_pos, false);

            if (loop_end) {

                let end_pos = new Position(loop_end.range.startLineNumber, loop_end.range.startColumn);
    ...
    getCompletionForCurrentObject(suggestions, context, token) {
        ...
        if (!suggestions.length && this.getLastNExpression(1) == '.' && this.getLastCharacter() != '.') {
            ...
            let position = new Position(this.lineNumber, column);
                        ...
                        if (!this.getClassCompletion(suggestions, bslGlobals.classes, false)) {
                            ...
                            if (!this.getClassCompletion(suggestions, bslGlobals.systemEnum, false)) {
                                ...
                                if (!this.getMetadataCompletion(suggestions, bslMetadata)) {
                                    ...
                                    if (engLang)
                                        this.getCommonCompletion(suggestions, bslGlobals.keywords, languages.CompletionItemKind.Keyword, true);
                                    else
                                        this.getCommonCompletion(suggestions, bslGlobals.keywords, languages.CompletionItemKind.Keyword, true);
                                    ...
                                    if (this.requireClass()) {
                                        ...
                                        this.getCommonCompletion(suggestions, bslGlobals.globalfunctions, languages.CompletionItemKind.Function, true);
                                        this.getCommonCompletion(suggestions, bslGlobals.globalvariables, languages.CompletionItemKind.Class, true);
                                        this.getCommonCompletion(suggestions, bslGlobals.systemEnum, languages.CompletionItemKind.Enum, false);
                                        this.getCommonCompletion(suggestions, bslGlobals.customFunctions, languages.CompletionItemKind.Function, true);
                                        this.getCommonModulesCompletion(suggestions);
                                        this.getCustomObjectsCompletion(suggestions, bslMetadata.customObjects, languages.CompletionItemKind.Enum);
                                    }
    ...
    getCompletion(context, token) {
        ...
        if (!suggestions.length && !editor.disableNativeSuggestions) {
            ...
            if (!this.isItStringLiteral()) {
                ...
                if (this.requireType())
                    this.getTypesCompletion(suggestions, bslGlobals.types, languages.CompletionItemKind.Enum);
            }
    ...
    resolveCompletionItem(completionItem) {
        ...
        if (completionItem.kind == languages.CompletionItemKind.Snippet) {
    ...
    getQueryFieldsCompletionForTempTable(suggestions, sourceDefinition, startPosition) {
        ...
        if (intoMatch) {
            ...
            let position = new Position(intoMatch.range.startLineNumber, intoMatch.range.startColumn);
            ...
            if (startMatch) {
                ...
                let searchRange = new Range(startMatch.range.startLineNumber, 1, intoMatch.range.startLineNumber, 1);
    ...
    getQueryFieldsChainCompletion(suggestions) {
        ...
        let match = Finder.findMatches(this.model, '[a-zA-Z0-9\u0410-\u044F]+', new Range(this.lineNumber, 1, this.lineNumber, this.column));
        ...
        if (match.length) {
            ...
            for (let i = 0; i < match.length; i++) {
                ...
                this.position = new Position(this.lineNumber, field_range.endColumn + 1);
    ...
    getQuerySourcesUntilPosition() {
        ...
        if (startMatch) {
            ...
            let position = new Position(startMatch.range.startLineNumber, startMatch.range.startColumn);
    ...
    getQueryFieldsCompletion(suggestions, allowChain = true) {
        ...
        if (this.getLastCharacter() == '.' && last_expression)
            ...
            position = new Position(this.lineNumber, this.column - last_expression.length);
                ...
                if (match) {
                    ...
                    if (match) {
                        ...
                        if (bracket_match && match.range.startLineNumber < bracket_match.range.startLineNumber) {
                            position = new Position(bracket_match.range.startLineNumber, bracket_match.range.startColumn);
                            let brackets = editor.getModel().matchBracket(position);
                            if (brackets) {
                                brackets = brackets.sort();
                                const open = brackets[0], close = brackets[1];
                                const bracket_range = new Range(open.startLineNumber, open.endColumn, close.startLineNumber, close.startColumn);
                                ...
                                const source_model = editor.createModel(source_text);
                                const model_range = source_model.getFullModelRange();
                                const model_position = new Position(model_range.endLineNumber, model_range.endColumn);
        ...
        if (this.isSuitablePlaceForQueryTable()) {
            ...
            if (startMatch) {
                ...
                if (fromMatch) {
                    ...
                    let range = new Range(fromMatch.range.startLineNumber, 1, fromMatch.range.endLineNumber, fromMatch.range.endColumn);
    ...
    getQueryAliasCompletion(suggestions) {
        ...
        if (this.isSuitablePlaceForQueryAlias()) {
            ...
            while (!this.model.getWordUntilPosition(position).word && 1 < position.column) {
                position = new Position(position.lineNumber, position.column - 1);
            }
        ...
        if (trigger_char == ' ') {
            ...
            if (!suggestions.length && !editor.disableNativeSuggestions) {
                ...
                if (this.requireQueryValue()) {
                    this.getQueryValuesCompletion(suggestions, bslQuery.values, languages.CompletionItemKind.Enum);
                    interrupt = true;
                }

                if (!interrupt && this.requireQueryRef()) {
                    this.getQueryRefCompletion(suggestions, languages.CompletionItemKind.Enum);
                    interrupt = true;
                }
                ...
                if (!interrupt && !this.getQuerySourceCompletion(suggestions, languages.CompletionItemKind.Enum)) {
                    ...
                    if (this.lastOperator != '"') {
                        ...
                        this.getRefCompletion(suggestions);
                        this.getQueryTablesCompletion(suggestions, languages.CompletionItemKind.Class);
                        this.getCustomObjectsCompletion(suggestions, bslMetadata.customObjects, languages.CompletionItemKind.Enum);
                    }

                    if (trigger_char == '&')
                        this.getQueryParamsCompletion(suggestions, languages.CompletionItemKind.Enum);

                    this.getQueryFieldsCompletion(suggestions);

                    if (this.lastExpression.indexOf('.') < 0) {
                        this.getQueryCommonCompletion(suggestions, languages.CompletionItemKind.Module);
                        if (functions)
                            this.getCommonCompletion(suggestions, functions, languages.CompletionItemKind.Function, true);
                    }
    ...
    getDCSCompletion() {
        ...
        if (!suggestions.length && !editor.disableNativeSuggestions) {
            ...
            if (!this.requireQueryValue()) {
                ...
                if (this.lastOperator != '"') {
                    this.getFillSuggestionsFromArray(suggestions, languages.bsl.languageDef.rules.DCSExp, languages.CompletionItemKind.Module, false);
                    this.getFillSuggestionsFromArray(suggestions, languages.dcs.languageDef.rules.characteristics, languages.CompletionItemKind.Module, false);
    ...
    getLastSigMethod(context) {
        ...
        if (bracket && this.isSuitablePlaceForSigHelp()) {
            ...
            let position = new Position(bracket.startLineNumber, bracket.startColumn);
            let data = this.model.getWordUntilPosition(position);

            while (data.word) {
                method = data.word + method;
                let range = new Range(position.lineNumber, data.startColumn - 1, position.lineNumber, data.startColumn);
                let last_char = this.getLastCharacter(range);
                if (last_char == '.') {
                    method = '.' + method;
                    position = new Position(position.lineNumber, data.startColumn - 1);
            ...
            let range = new Range(bracket.startLineNumber, bracket.startColumn + 1, this.lineNumber, this.column);
    ...
    static parseFunctionDescription(model, funcLineNumber) {
        ...
        while (0 < line_number && model.getValueInRange(new Range(line_number, 1, line_number, 3)) == '//') {
            line_number--;
        }

        line_number++;

        const matches = Finder.findMatches(model, 'параметры:', new Range(line_number, 1, funcLineNumber, 1));

        if (matches && matches.length) {
            let range = new Range(line_number, 1, matches[0].range.startLineNumber, matches[0].range.startColumn);
            short_description = model.getValueInRange(range);
        }
        else
            short_description = model.getValueInRange(new Range(line_number, 1, funcLineNumber, 1));

        full_description = model.getValueInRange(new Range(line_number, 1, funcLineNumber, 1))
    ...
    static parseFunctionParameters(model, parametersStr, funcLineNumber) {
        ...
        if (parametersStr) {
            ...
            while (0 < line_number && !paramsExist && model.getValueInRange(new Range(line_number, 1, line_number, 3)) == '//') {
                line_number--;
                paramsExist = (model.getValueInRange(new Range(line_number, 1, line_number, 3)) == '// Параметры:');
            }

            line_number++;

            const params = parametersStr.split(',');
            const range = new Range(line_number, 1, funcLineNumber, 1);
            const func_model = editor.createModel(model.getValueInRange(range));
    ...
    static parseModule(moduleText) {
        ...
        const model = editor.createModel(moduleText);
    ...
    static getRangesForQueryBlock(model, regexp, scopes, includeStartString, includeEndString) {
        ...
        if (scopes) {
            ...
            while (scope_idx < scopes.length) {
                ...
                matches = matches.concat(Finder.findMatches(model, regexp, new Range(scope.start, 1, scope.end + 1, 1)));
                scope_idx++;
            }
    ...
    getCustomHover() {
        ...
        for (const [key, value] of Object.entries(customHovers)) {
            ...
            if (key.toLowerCase() == this.word && value && !this.isItStringLiteral()) {
                ...
                return {
                    range: new Range(this.lineNumber, this.column, this.lineNumber, this.model.getLineMaxColumn(this.lineNumber)),
                    contents: contents
                };
    ...
    addPrefix(prefix) {
        ...
        if (0 < maxLine)
            editor.setSelection(new Range(selection.startLineNumber, 1, maxLine, this.model.getLineMaxColumn(maxLine)));
    ...
    static setText(txt, range, usePadding) {
        ...
        if (range) {
            if (typeof range === 'string') {
                ...
                insertRange = new Range(rangeObject.startLineNumber, rangeObject.startColumn, rangeObject.endLineNumber, rangeObject.endColumn)
            } else {
                insertRange = range
            }
        } else {
            insertRange = Range.fromPositions(editor.getPosition())
        }
        ...
        if (usePadding && 1 < startColumn) {
            ...
            let valueBefore = editor.getModel().getValueInRange(
                new Range(insertRange.startLineNumber, 1, insertRange.startLineNumber, startColumn)
            );
    ...
    static provideColorPresentations(model, colorInfo) {
        ...
        let match = Finder.findNextMatch(model, pattern, new Position(range.startLineNumber, range.startColumn));
    ...
    getDefinition() {
        ...
        if (this.word) {
            ...
            if (exp_arr.length == 1) {
                ...
                let position = new Position(this.lineNumber, 1);
                ...
                if (!definition) {
                    ...
                    position = new Position(this.lineNumber, 999);
    ...
    getQueryFieldDefinition() {
        ...
        if (match && match.range.startLineNumber < this.lineNumber) {
            ...
            let match_position = new Position(match.range.startLineNumber, match.range.startColumn);
    ...
    getQuerySourceDefinition() {
        ...
        let location = null;
        let pattern = '(as|как)\\s*' + this.word;
        let position = new Position(this.lineNumber, this.model.getLineMaxColumn(this.lineNumber));
